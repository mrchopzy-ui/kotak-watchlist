...
search.addEventListener("input", async () => {
    ...
    // If a previous search is still loading, cancel it so it doesn't overwrite our new results
    if (searchAborter) searchAborter.abort();
    searchAborter = new AbortController();

    try {
        const res = await fetch(`/search?q=${query}`, { signal: searchAborter.signal });
        ...
        // Ignored. This just means we successfully canceled an old request.
    }
});
...
async function loadPrices() {
    ...
    // Sort the fetched data array before generating HTML
